...
import com.dereekb.gae.utilities.deprecated.function.staged.components.StagedFunctionStageMap;
...
public final class StagedFunctionObserverMap<T, W extends StagedFunctionObject<T>> extends StagedFunctionStageMap<StagedFunctionObserver<T>, StagedFunctionObjectObserver<T, W>> {
	...
	public void notifyObserversOfStage(StagedFunctionStage stage,
		...
		List<StagedFunctionObserver<T>> observers = this.normal.valuesForKey(stage);
		List<StagedFunctionObjectObserver<T, W>> objectObservers = this.objectDependent.valuesForKey(stage);
