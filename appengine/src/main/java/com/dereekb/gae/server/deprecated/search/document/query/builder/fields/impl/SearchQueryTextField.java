 ...
 */
@Deprecated
public class SearchQueryTextField extends SearchQueryFieldImpl {
	...
	public SearchQueryTextField(String field, String value) {
		super(FieldType.TEXT);
		this.value = value;
		this.field = field;
	}

	public SearchQueryTextField(String field, String value, Boolean not) {
	...
	protected SearchQueryFieldImpl cloneQuery() {
		...
		SearchQueryTextField clone = new SearchQueryTextField(this.field, this.value, this.isNot());
	...
	protected String getQueryString() {
		...
		if (this.specificText) {
			queryString = String.format(SPECIFIC_TEXT_FIELD_FORMAT, this.field, this.value);
		} else {
			String stem = (this.stemming) ? STEM_SYMBOL : "";
			queryString = String.format(TEXT_FIELD_FORMAT, this.field, stem, this.value);
		}
